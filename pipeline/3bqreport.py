...
SERVICE_KEY_PATH = os.environ.get("GCS_CREDENTIALS",  "")
...
def _bq_client():
    ...
    if SERVICE_KEY_PATH and os.path.exists(SERVICE_KEY_PATH):
        creds = sa.Credentials.from_service_account_file(SERVICE_KEY_PATH)
        return bigquery.Client(project=PROJECT_ID, credentials=creds,
                               location=BQ_LOCATION)

    print(f"[WARN] Service key not found at '{SERVICE_KEY_PATH}' - falling back to ADC.")
    from google.cloud import bigquery
...
def upload_to_gcs(local_pdf: str, drug_name: str,
    ...
    if SERVICE_KEY_PATH and os.path.exists(SERVICE_KEY_PATH):
        ...
        print(f"  [WARN] Service key not found at '{SERVICE_KEY_PATH}' - using ADC for GCS.")
...
def generate_report(api_key: str = "", gcs_bucket: str = "cognito-gcs"):
    ...
    if not SERVICE_KEY_PATH:
        raise EnvironmentError(
            "GCS_CREDENTIALS is not set. Add it to your .env file:\n"
            "  GCS_CREDENTIALS=/path/to/service-account.json"
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--key-file", default="",
                        help="Service account JSON path (overrides GCS_CREDENTIALS)")
    ...
    if args.key_file:         SERVICE_KEY_PATH      = args.key_file
