...
def _require_credentials():
    """Validate that the service-account credential path is available."""
...
def _load_from_bigquery() -> pd.DataFrame:
    print(f"  Authenticating with service account: {GCS_CREDENTIALS}")
...
def _get_bq_client():
    """Return an authenticated BigQuery client (reuses service-account creds)."""
    try:
        from google.cloud import bigquery
        from google.oauth2 import service_account
    except ImportError:
        sys.exit("ERROR: google-cloud-bigquery and google-auth are required.")
