...
BQ_SERVICE_KEY = os.getenv("SERVICE_KEY_PATH", "")
...
def load_data_from_bigquery() -> pd.DataFrame:
    ...
    Uses SERVICE_KEY_PATH for authentication if provided, else ADC.
    ...
    if BQ_SERVICE_KEY and os.path.exists(BQ_SERVICE_KEY):
        # No custom scopes — let the library use its defaults
...
def _get_bq_client() -> bigquery.Client:
    """Return an authenticated BigQuery client (reuses hardcoded config)."""
    if BQ_SERVICE_KEY and os.path.exists(BQ_SERVICE_KEY):
        credentials = service_account.Credentials.from_service_account_file(BQ_SERVICE_KEY)
