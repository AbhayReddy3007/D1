...
BQ_SERVICE_KEY = r"C:\Users\p90022569\Downloads\Cognito 1\Cognito\cognito-prod-394707-750a8b798947.json"
...
def _get_bq_client() -> bigquery.Client:
    ...
    if BQ_SERVICE_KEY and os.path.exists(BQ_SERVICE_KEY):
    ...
    print("  ⚠  Service key not found — using Application Default Credentials.")
...
def upload_to_gcs(local_pdf: str, drug_name: str,
    ...
    if BQ_SERVICE_KEY and os.path.exists(BQ_SERVICE_KEY):
        ...
        creds  = sa.Credentials.from_service_account_file(BQ_SERVICE_KEY)
        client = storage.Client(project=BQ_PROJECT_ID, credentials=creds)
    else:
        print(f"  ⚠  Service key not found at '{BQ_SERVICE_KEY}' — using ADC for GCS.")
